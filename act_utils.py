...
def sample_insertion_unreachable():
    np.random.seed()
    ...
    # peg_quat = np.array([1, 0, 0, 0])
    peg_yaw = np.random.uniform(-np.pi, np.pi)
    peg_quat = np.array([np.cos(peg_yaw/2), 0, 0, np.sin(peg_yaw/2)])
    peg_pose = np.concatenate([peg_position, peg_quat])
    ...
    while True:
        ...
        if np.linalg.norm(peg_xy - socket_xy) > 0.15:
            break

    # socket_quat = np.array([1, 0, 0, 0])
    socket_yaw = np.random.uniform(-np.pi, np.pi)
    socket_quat = np.array([np.cos(socket_yaw/2), 0, 0, np.sin(socket_yaw/2)])
    socket_pose = np.concatenate([socket_position, socket_quat])
